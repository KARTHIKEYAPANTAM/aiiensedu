...
import { isOAuthCallbackUrl } from './postAuthRouter.js';
...
export function AuthProvider({ children }) {
  ...
  const pendingAuthEventRef = React.useRef(null);
    ...
    async function init() {
      ...
      try {
        if (isOAuthCallbackUrl()) {
          window.__AIMEASY_SUPABASE__ = window.__AIMEASY_SUPABASE__ || supabase;
          window.__aimeasyAuthRestoring = true;
          invalidateSessionCache();
          try {
            await withAuthTimeout(
              window.__aimeasyOAuthCallbackPromise || completeOAuthCallbackFromUrl(),
              'AuthProvider.oauthCallback',
            );
          } catch (oauthError) {
            console.warn('[AUTH] OAuth callback completion failed', {
        ...
        if (mounted) {
          ...
          if (pendingAuthEvent?.nextSession) {
            pendingAuthEventRef.current = null;
            completedHasSession = Boolean(pendingAuthEvent.nextSession);
            setSession(pendingAuthEvent.nextSession);
            try {
              await refreshProfile(pendingAuthEvent.nextSession);
            } catch (e) {
              console.warn('[AUTH] Profile restore from pending auth event failed', {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange(async (event, nextSession) => {
      ...
      if (!initCompleteRef.current) {
        if (event !== 'INITIAL_SESSION' && nextSession) {
          pendingAuthEventRef.current = { event, nextSession };
        }
        return;
      }
