...
async function applyRoute(path) {
  ...
  if (path === '/auth' && isOAuthCallbackHash()) {
    ...
    const ok = await requestAuthSync({ reason: 'navigation-oauth-callback' });
    oauthNavigationHandled = Boolean(ok);
    return;
  }
...
export function installBrowserNavigation() {
  ...
  window.addEventListener('hashchange', async () => {
    if (isOAuthCallbackHash()) {
      ...
      const ok = await requestAuthSync({ reason: 'hashchange-oauth-callback' });
      oauthNavigationHandled = Boolean(ok);
      return;
    }
...
export async function startBrowserNavigation() {
  ...
  if (isOAuthCallbackHash()) {
    ...
    if (!oauthNavigationHandled) {
      window.showScreen?.('screen-setting-up-profile');
      const ok = await requestAuthSync({ reason: 'start-navigation-oauth-callback' });
      oauthNavigationHandled = Boolean(ok);
    }
    return;
  }
